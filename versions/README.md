...
- Music obstacle generator: current root has `src/obstacleGenerator.js` and `OBSTACLE_GENERATOR` tuning so bass/treble/volume/hit values can influence candidate obstacles while safety checks reject overlap, crowding, too-frequent Jumpwaves, too many generated obstacles, and unsafe lane coverage.
