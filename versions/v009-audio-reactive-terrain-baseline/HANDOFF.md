...
- Optional `assets/audio/test.ogg` analysis that drives visual-only wave height, terrain pulse, shimmer, and grid glow. `test.wav` remains a local fallback.
...
- `src/audio.js` analyzes one mixed track from `assets/audio/test.ogg` or fallback `test.wav`; current audio values affect visuals only, not terrain generation or timing spawns.
